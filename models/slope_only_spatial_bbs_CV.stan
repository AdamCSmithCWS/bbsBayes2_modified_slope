// This is a modified version of the model = "slope" model_variant = "spatial" 
// from teh R package bbsBayes2 https://github.com/bbsBayes/bbsBayes2
// it is a Stan implementation of the slope model that 
// shares information among strata on the intercepts and slopes
// Spatial
...
// intercepts and slopes
  ...
  for(i in 1:n_train){
    ...
    E[i] =  beta[strat_tr[i]]*(year[i] - fixed_year) + strata + eta*first_year_tr[i] + ste + obs + noise;
  }
...
//   array[n_strata*calc_n2,n_years*calc_n2] real<lower=0> n_slope2; //smooth component of annual indices calculated assuming site-effects are log-normal and the same among strata
  ...
  if(calc_CV){
   ...
   log_lik_cv[i] = poisson_log_lpmf(count_te[i] | beta[strat_te[i]]*(year_te[i] - fixed_year) + strata + + eta*first_year_te[i] + ste + obs + noise);

   }else{
     noise = 0;
  log_lik_cv[i] = neg_binomial_2_log_lpmf(count_te[i] | beta[strat_te[i]]*(year_te[i] - fixed_year) + strata + eta*first_year_te[i] + ste + obs + noise, phi);
...
for(y in 1:n_years){
  ...
  real obs = sdobs*obs_raw[obs_mat[s,t]]; // observer intercepts
      ...
      n_t[t] = exp(strata + beta[s]*(y-fixed_year) + retrans_noise + ste + obs);
        }
